...
const toRoomId = (conversationId) => String(conversationId);
...
module.exports = (io, socket) => {
  ...
  socket.on("join_conversation", async (payload) => {
    const conversation_id = getConversationId(payload);
    if (!conversation_id) {
      return socket.emit("error", { message: "conversation_id is required" });
    }

    const roomId = toRoomId(conversation_id);

    try {
      const result = await queryHandler.getConversationById({
        conversation_id,
        user_id: userId,
      });
      if (result.err) {
      ...
      logger.error(ctx, "join_conversation failed", "chatHandler", err);
  ...
  socket.on("leave_conversation", (payload) => {
    ...
    socket.leave(toRoomId(conversation_id));
  ...
  socket.on("send_message", async (payload) => {
    ...
    try {
      ...
      io.to(roomId).emit("receive_message", result.data);
      ...
      logger.error(ctx, "send_message failed", "chatHandler", err);
  ...
  socket.on("typing", (payload) => {
    ...
    socket.to(toRoomId(conversation_id)).emit("typing", { user_id: userId, conversation_id });
  });
  ...
  socket.on("stop_typing", (payload) => {
    ...
    socket.to(toRoomId(conversation_id)).emit("stop_typing", { user_id: userId, conversation_id });
  });
  ...
  socket.on("read_message", async (payload) => {
    ...
    try {
      ...
      socket.to(roomId).emit("read_message", {
        conversation_id,
        reader_id: userId,
      });
      ...
      logger.error(ctx, "read_message failed", "chatHandler", err);
